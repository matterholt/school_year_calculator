...
function addToStudentCourses (){
  ...
  studentCourseWork = [...studentCourseWork, {subject_matter: subjectMatter,number_of_chapters: totalLessons}]
   subjectMatter = ''
 totalLessons = ''
...
<div class="student_classlist">
    {#if studentCourseWork.length !== 0}
          ...
          <div class="student_courses">
              {#if pupilStatus === "edit"}
         <button  class="student_activateEdit" on:click={() => console.log("edit mode")} >:::</button>
  {/if}
  ...
  {/if}
    {#if pupilStatus === "edit"}
  <div>
    <input type="text" placeholder="subject matter" id="subjectMatter" bind:value={subjectMatter} />
    <input  type="number" placeholder="Total lessons"

    id="totalLessons" bind:value={totalLessons} min="1"
    />
